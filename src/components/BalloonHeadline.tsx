export function BalloonHeadline({ theme = 'dark' }: { theme?: 'dark' | 'light' }) {
  const isDark = theme === 'dark';
  const fillColor = isDark ? '#ffffff' : '#0a0a0a';
  const specColor = isDark ? '#ffffff' : '#b0b0b0';
  const wrinkleColor = isDark ? '#ffffff' : '#d0d0d0';
  const diffuseColor = isDark ? '#6e6e6e' : '#4d4d4d';
  const shadowColor = isDark ? '#000000' : '#000000';
  const shadowOpacity = isDark ? '0.35' : '0.45';
      ...
      <svg viewBox="0 0 1000 340" xmlns="http://www.w3.org/2000/svg" role="img" className="w-full h-auto animate-reveal">
        <defs>
          <filter id="balloon" x="-35%" y="-90%" width="170%" height="280%" colorInterpolationFilters="sRGB">
            ...
            <feSpecularLighting in="hf" surfaceScale="14" specularConstant="1.25" specularExponent="8" lightingColor={specColor} result="spec">
              <fePointLight x="400" y="-160" z="240"/>
            </feSpecularLighting>
            <feDiffuseLighting in="hf" surfaceScale="12" diffuseConstant="1.15" lightingColor={diffuseColor} result="diff">
            ...
            <feSpecularLighting in="hfine" surfaceScale="4" specularConstant="0.4" specularExponent="15" lightingColor={wrinkleColor} result="wr">
            ...
            <feMerge result="merged"><feMergeNode in="dIn"/><feMergeNode in="sIn"/><feMergeNode in="wIn"/></feMerge>
            <feDropShadow in="merged" dx="0" dy="10" stdDeviation="12" floodColor={shadowColor} floodOpacity={shadowOpacity} result="balloonFinal"/>
          </filter>
        </defs>
